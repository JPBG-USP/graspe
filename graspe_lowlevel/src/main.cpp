...
//== LEFT JOYSTICK X axis - - - Theta ==//
  if (abs(ctl->axisX())>abs(ctl->axisY())){
    if (ctl->axisX() >= 80 || ctl->axisX() <= -80) {
        int d_angulo_base = map(ctl->axisX(),-512,512,10,-10);
        graspe_manipulator.joint1.set_angle(graspe_manipulator.joint1.get_angle_current()+degrees_to_rad((float)d_angulo_base));
    }
  }
  ...
  if (abs(ctl->axisX())<abs(ctl->axisY())){
    if (ctl->axisY() >= 80 || ctl->axisY() <= -80) {
      int d_angulo_2= map(ctl->axisY(),-512,512,5,-5);
      graspe_manipulator.joint2.set_angle(graspe_manipulator.joint2.get_angle_current()+degrees_to_rad((float)d_angulo_2));
    }
  }
  ...
  if(abs(ctl->axisRX())<abs(ctl->axisRY())){
    if (ctl->axisRY() >= 150 || ctl->axisRY() <= -150) {
      Serial.println("Li o joy direito");
      int d_angulo_3= map(ctl->axisRY(),-512,512,10,-10);
      Serial.println(d_angulo_3);
      Serial.println(degrees_to_rad((float)d_angulo_3));
      Serial.println(graspe_manipulator.joint4.get_angle_current()+degrees_to_rad((float)d_angulo_3));
      graspe_manipulator.joint3.set_angle(graspe_manipulator.joint3.get_angle_current()+degrees_to_rad((float)d_angulo_3));
    } 
  }
  ...
  if (abs(ctl->axisRX())>abs(ctl->axisRY())){
    if (ctl->axisRX() >= 80 || ctl->axisRX() <= -80) {
      Serial.println("Li o joy direito");
      int d_angulo_4= map(ctl->axisRX(),-512,512,-10,+10);
      Serial.println(d_angulo_4);
      Serial.println(degrees_to_rad((float)d_angulo_4));
      Serial.println(graspe_manipulator.joint4.get_angle_current()+degrees_to_rad((float)d_angulo_4));
      graspe_manipulator.joint4.set_angle(graspe_manipulator.joint4.get_angle_current()+degrees_to_rad((float)d_angulo_4));
    }
  }
  Serial.printf("%f/%f/%f/%f\n",
    graspe_manipulator.joint1.get_angle_current(),
...
void processGamepad(ControllerPtr ctl) {
  ...
  if(!joint_flag){
    ...
    Serial.println("cylindrical control active");
  }
  if(joint_flag){
    joint_control(ctl);
    Serial.println("joint control active");
  }
  //dumpGamepad(ctl);
}
...
void setup() {
  ...
  joint_flag = digitalRead(joint_flag_pin);
