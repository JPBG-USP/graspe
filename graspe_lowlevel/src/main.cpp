...
//== LEFT JOYSTICK X axis - - - Theta ==//
  ...
  if (abs(ctl->axisX())>abs(ctl->axisY())){
    if (ctl->axisX() >= 80 || ctl->axisX() <= -80) {
        int d_base_angle = map(ctl->axisX(),-512,512,10,-10);
  ...
  if (abs(ctl->axisRX())>abs(ctl->axisRY())){
    if (ctl->axisRX() >= 80 || ctl->axisRX() <= -80) {
      int d_approach_angle = map(ctl->axisRX(),-512,512,10,-10);
      delta_pos[3] = degrees_to_rad(float(d_approach_angle));
    }
  }

  graspe_manipulator.set_pose(delta_pos);
...
//== LEFT JOYSTICK X axis - - - Theta ==//
  ...
  if (abs(ctl->axisX())>abs(ctl->axisY())){
    if (ctl->axisX() >= 80 || ctl->axisX() <= -80) {
        d_angle_1 = map(ctl->axisX(),-512,512,10,-10);
  ...
  if(abs(ctl->axisRX())<abs(ctl->axisRY())){
    if (ctl->axisRY() >= 80 || ctl->axisRY() <= -80) {
      d_angle_3 = map(ctl->axisRY(),-512,512,10,-10);
      graspe_manipulator.joint3.set_angle(graspe_manipulator.joint3.get_angle_current()+degrees_to_rad((float)d_angle_3));
    } 
  }
  ...
  if (abs(ctl->axisRX())>abs(ctl->axisRY())){
    if (ctl->axisRX() >= 80 || ctl->axisRX() <= -80) {
      d_angle_4 = map(ctl->axisRX(),-512,512,-10,+10);
...
void processGamepad(ControllerPtr ctl) {
  // Flag for joint control mode (AKA debug mode) via HIGH signal on pin 15
  if(!joint_flag){
